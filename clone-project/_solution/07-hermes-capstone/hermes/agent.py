...
from .trace import log_event   # M7 — 관찰(observability): 도구 호출을 logs/trace.jsonl에 남긴다
            ...
            for block in resp.content:
                if block.type == "tool_use":
                    ...
                    if block.name in DANGEROUS_TOOLS and not confirm(block.name, block.input):
                        ...
                        log_event(step, block.name, block.input, DENIED_MSG, approved=False)  # M7 [+] 거부도 관찰
                    ...
                    log_event(step, block.name, block.input, result, approved=True)  # M7 [+] 실행됨을 관찰
                    tool_results.append({
                        "type": "tool_result",
                        "tool_use_id": block.id,   # tool_use 의 id와 반드시 매칭
