...
# M5: persona(말투) 뒤에 '행동 정책' 문단들이 붙었다 — system은 history 밖 항구 규칙이라
# compaction(M4)에도 안 사라지므로 정책을 두기 좋은 자리다.
# M6: 그 위에 'grounding 정책' 한 문단이 더 얹혔다(먼저 검색 / 없으면 모른다고).
# ⚠️ 단, 이건 전부 '부탁'이다. 모델이 안 지킬 수도 있다. 강제는 코드가 한다 —
#    agent.py의 승인 게이트(DANGEROUS_TOOLS)가 진짜 guardrail이고, search_docs는
#    위험하지 않아 강제할 필요가 없어 '부탁'으로 둔다.
...
프로젝트나 로컬 문서에 있을 법한 사실(사내 도구·설정·규칙 등)을 물으면, 추측하지 말고 먼저
search_docs 도구로 근거를 찾은 뒤 그 내용에 기반해 답한다. 검색 결과에 답이 없으면 모른다고 말한다."""
